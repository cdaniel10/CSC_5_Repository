...
//Function Prototypes
void savings(float [],int [],int[],int,float,float &);
...
int main(int argc, char** argv) {
    ...
    savings(balance,year,dYear,NYEARS,yrDep,intRate);
    
    //Display the Heading
    display(balance,year,dYear,NYEARS,intRate,salary,yrDpPc,yrDep);

    //Exit stage right!
    return 0;
}

void display(float balance[],int year[],int dYear[],int NYEARS,
             float intRate, float salary, float yrDpPc, float yrDep){
...
void savings(float balance[],int year[],int dYear[],int NYEARS,float yrDep,float &intRate){
    //Process or map Inputs to Outputs
    intRate/=CNVDCML;
