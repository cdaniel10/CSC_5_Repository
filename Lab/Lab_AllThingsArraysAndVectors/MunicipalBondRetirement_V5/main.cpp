...
//Function Prototypes
void savings(vector<float>&,vector<int>&,vector<int>&,float &,float);
...
int main(int argc, char** argv) {
    ...
    savings(balance,year,dYear,intRate,yrDep);
...
void display(vector<float> &balance,vector<int> &year,
             vector<int> &dYear,int NYEARS,float intRate,
             float salary,float yrDpPc,float yrDep){
    //Display the Heading
...
void savings(vector<float> &balance,vector<int> &year,
             vector<int> &dYear,float &intRate,float yrDep){
    //Process or map Inputs to Outputs
